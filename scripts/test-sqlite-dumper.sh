...
while IFS= read -r wrapper_file; do
  ...
  # `if ! ensure_...` is fine (it fails closed in the branch); a BARE `! ensure_...`
  # is not — it inverts the guard's result while reading like a delegation. The real
  # wrapper is a plain `ensure_sqlite_dumper || { ... }`, so disallowing the bare
  # negation costs nothing. (Carnot, #186 round 6 — a finding generated by round 5's
  # own regex, which is the documented signal to stop after taking it.)
